...
* for finding all-trichord row generators.
*
...
static int isValidTrichordRow(int* permutation, int trichordTable[12][12])
{
...
static void loadIntervalTable(int table[12][12])
{
	table[2][11] = 1;
	table[11][2] = 1;
	table[1][10] = 1;
...
static void nextTrichordRow(int* permutation, int trichordTable[12][12])
{
...
static void writeTrichordRowsToFile(std::string path, std::vector<int*> found)
{
	std::ofstream file;
	file.open("D:\\all_trichord.json");
	file << "{\n    \"allTrichordRowGenerators\": [\n";
